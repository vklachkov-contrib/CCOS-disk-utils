...
int ccos_get_dir_contents(ccos_disk_t* disk, ccos_inode_t* dir, uint16_t* entry_count, ccos_inode_t*** entries) {
  ...
  res = ccos_read_file(disk, dir, &dir_contents, &dir_size);
  if (res) {
  ...
  *entry_count = dir->desc.dir_count;
  ...
  res = parse_directory_data(disk, dir_contents, dir_size, *entry_count, &elements);
  if (res) {
...
int ccos_replace_file(ccos_disk_t* disk, ccos_inode_t* file, const uint8_t* file_data, uint32_t file_size) {
  ...
  res = get_file_blocks(disk, file, &block_count, &blocks);
  if (res) {
  ...
  for (size_t i = 0; i < block_count; ++i) {
    ...
    res = get_block_data(disk, blocks[i], &start, &data_size);
    if (res) {
...
int ccos_read_file(ccos_disk_t* disk, ccos_inode_t* file, uint8_t** file_data, size_t* file_size) {
  ...
  int res = get_file_blocks(disk, file, &blocks_count, &blocks);
  if (res) {
  ...
  for (int i = 0; i < blocks_count; ++i) {
    ...
    res = get_block_data(disk, blocks[i], &chunk, &chunk_size);
    if (res) {
...
int ccos_write_file(ccos_disk_t* disk, ccos_inode_t* file, const uint8_t* file_data, size_t file_size) {
  ...
  res = get_file_blocks(disk, file, &blocks_count, &blocks);
  if (res) {
  ...
  if (out_blocks_count > blocks_count) {
    ...
    for (int i = 0; i < (blocks_count - out_blocks_count); ++i) {
      ...
      res = remove_block_from_file(disk, file, &bitmask_list);
      if (res) {
  ...
  for (int i = 0; i < blocks_count; ++i) {
    ...
    get_block_data(disk, blocks[i], &block, &size);
...
int ccos_delete_file(ccos_disk_t* disk, ccos_inode_t* file) {
  ...
  int res = delete_file_from_parent_dir(disk, file);
  if (res) {
  ...
  while (file->content_inode_info.block_next != CCOS_INVALID_BLOCK) {
    res = remove_content_inode(disk, file, &bitmask_list);
    if (res) {
...
ccos_inode_t* ccos_add_file(ccos_disk_t* disk, ccos_inode_t* dest_directory, uint8_t* file_data, size_t file_size, const char* file_name) {
  ...
  TRACE("Writing file 0x%lx", new_file->header.file_id);
  int res = ccos_write_file(disk, new_file, file_data, file_size);
  if (res) {
    fprintf(stderr, "Unable to write file to file with id 0x%x!\n", new_file->header.file_id);
    return NULL;
  }

  res = add_file_to_directory(disk, dest_directory, new_file);
...
ccos_inode_t* ccos_get_root_dir(ccos_disk_t* disk) {
  ...
  int res = get_superblock(disk, &superblock);
  if (res) {
...
int ccos_calc_free_space(ccos_disk_t* disk, size_t* free_space) {
  uint16_t superblock = 0;

  int ret = get_superblock(disk, &superblock);
  if (ret) {
...
ccos_inode_t* ccos_create_dir(ccos_disk_t* disk, ccos_inode_t* parent_dir, const char* directory_name) {
  ...
  size_t directory_size = get_dir_default_size(disk);
  ...
  ccos_inode_t* new_directory = ccos_add_file(disk, parent_dir, directory_contents, directory_size, filename);
  free(filename);
...
int ccos_rename_file(ccos_disk_t* disk, ccos_inode_t* file, const char* new_name, const char* new_type) {
  ...
  if (!is_root_dir(file)) {
    ...
    res = delete_file_from_parent_dir(disk, file);
    if (res) {
    ...
    res = add_file_to_directory(disk, parent_dir, file);
    if (res) {
