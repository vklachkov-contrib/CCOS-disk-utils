...
static ccos_bitmask_list_t init_bitmask(ccos_disk_t* disk, bitmask_info_t info) {
  ...
  for (size_t i = 0; i < info.count; i++) {
    ccos_bitmask_t* bitmask = ccos_disk_read_sector(disk, info.sector + i);
    assert(bitmask != NULL);
...
static void write_superblock(ccos_disk_t* disk, ccos_bitmask_list_t* bitmask_list) {
  ...
  ccos_inode_t* root_dir = ccos_disk_read_sector(disk, id);
  ...
  ccos_block_header_t* superblock_entry = ccos_disk_read_sector(disk, superblock_entry_block);
  assert(superblock_entry != NULL);
...
static void write_boot_code(ccos_disk_t* disk, disk_format_t format, ccos_bitmask_list_t* bitmask_list) {
  ...
  for (size_t i = 0; i < pages; i++) {
    memcpy(ccos_disk_read_sector(disk, offset + i), boot_code + i * disk->sector_size, disk->sector_size);
...
static void write_boot_sector(ccos_disk_t* disk, disk_format_t format, ccos_bitmask_list_t* bitmask_list) {
  ...
  for (size_t i = 0; i < pages; i++) {
    memcpy(ccos_disk_read_sector(disk, i), &boot_sector + i * disk->sector_size, disk->sector_size);
